...
const ChatSession = require("../models/MessageSchema"); // Adjust path as per your project
...
function getUserPrompt(query) {
  ...
  return `
Convert this natural language query to a MongoDB query or detect if it's small talk or email intent:
...
async function handleUserQuerys(req, res) {
  try {
    const sessionId = "123456"; // You may extract from req/session if needed
    ...
    // --- EMAIL CONFIRMATION HANDLING BEFORE AI ---
    if (
      ...
      if (userText === "yes") {
        logger.debug("User confirmed email sending");
        // Send emails
        let sent = 0;
      ...
      // Otherwise, you may forward to AI or handle differently
    }

    // --- AI intent detection & query/email drafting ---
    const aiResponse = await convertQueryToMongoDB(message);
    ...
    if (aiResponse.type === "email") {
      ...
      // Update AI reply with resolved recipient name if needed
    ...
    if (aiResponse.type === "query") {
      // Fallback: handle possible email intent missed by AI as in your previous flow
      ...
      // Run the MongoDB query with dynamic field selection
      try {
    ...
    // Fallback unknown AI response type
    session.history.push({
      role: "assistant",
      content:
        "I couldn't understand your request. Please try asking differently.",
    });
    session.lastActive = new Date();
    await session.save();
