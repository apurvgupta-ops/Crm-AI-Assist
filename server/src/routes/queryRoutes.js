...
router.post('/natural-language', validateQueryRequest, async (req, res) => {
  try {
    ...
    const { mongoQuery, explanation, suggestedFields, estimatedResults } = geminiResponse;
    ...
    logger.info(`Query executed successfully in ${executionTime}ms, returned ${results.length} results`);
...
router.post('/mongodb', async (req, res) => {
  try {
    ...
    logger.info(`Direct MongoDB query executed in ${executionTime}ms, returned ${results.length} results`);
